...
material material_create(vec3 color, vec2 albedo, float specular_exponent)
{
    material mat;
...
vec3 cast_ray(vec3 orig, vec3 dir, vec3 background_color, sphere *spheres, size_t spheres_len, light *lights, size_t lights_len)
{
    vec3 point, normal;
    material mat;

    if (!scene_intersect(orig, dir, spheres, spheres_len, &point, &normal, &mat))
    {
